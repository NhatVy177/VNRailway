...
-- =============================================
-- STORED PROCEDURE: usp_TaoDonDatVe_Online_FIXED
-- Fix: Lost Update Problem
-- =============================================
...
BEGIN
    ...
    --  FIX 1: Dùng SERIALIZABLE thay vì READ COMMITTED
    ...
    BEGIN TRY
        ...
        WHILE @@FETCH_STATUS = 0
        BEGIN
            -- LOCK NGAY KHI KIỂM TRA
            SELECT @SoGheDaDat = COUNT(*)
            FROM CHI_TIET_VE ctv WITH (UPDLOCK, HOLDLOCK)
            JOIN DON_DAT_VE ddv ON ctv.MaDon = ddv.MaDon
            WHERE ctv.MaToa = @MaToa 
              AND ctv.MaCho = @MaCho
              AND ddv.MaChuyenTau = @MaChuyenTau
              AND ctv.TrangThai <> N'Đã hủy'
              -- Check overlap ga đi/đến
              AND EXISTS (
                  SELECT 1 FROM CHUYEN_GA cg1, CHUYEN_GA cg2
                  WHERE cg1.MaChuyenTau = @MaChuyenTau
                    AND cg1.MaGa = ddv.MaGaDen
                    AND cg2.MaChuyenTau = @MaChuyenTau
                    AND cg2.MaGa = ddv.MaGaDi
                    AND NOT (cg1.TrinhTu <= @TrinhTuDi OR cg2.TrinhTu >= @TrinhTuDen)
              );
        ...
        -- FIX 3: XÓA DELAY
        WAITFOR DELAY '00:00:05';  -- Removed!
