...
-- =============================================
-- PROCEDURE: usp_TimKiemNhanVienCoThePhancong
-- Lấy danh sách nhân viên CÓ THỂ phân công với tìm kiếm theo tên hoặc mã
-- Bao gồm số giờ làm việc trong tuần
-- =============================================
CREATE OR ALTER PROC usp_TimKiemNhanVienCoThePhancong
    @MaChuyenTau NCHAR(10),
    @LoaiNhanVien NCHAR(2), -- 'LT' = Lái tàu, 'TT' = Toa tàu
    @TuKhoa NVARCHAR(100) = NULL, -- Từ khóa tìm kiếm (tên hoặc mã nhân viên)
...
BEGIN
    ...
    SET @TuKhoa = LTRIM(RTRIM(ISNULL(@TuKhoa, '')));
    
    -- Nếu từ khóa rỗng, đặt thành NULL để lấy tất cả
    IF @TuKhoa = ''
        SET @TuKhoa = NULL;
    ...
    WHERE 
        ...
        AND (
            @TuKhoa IS NULL -- Nếu không có từ khóa, lấy tất cả
            OR nv.MaNV LIKE '%' + @TuKhoa + '%' -- Tìm theo mã nhân viên
            OR nd.HoTen LIKE '%' + @TuKhoa + '%' -- Tìm theo tên
        )
        
    -- Sắp xếp theo số giờ làm việc tăng dần (ưu tiên người làm ít giờ)
    ORDER BY SoGioLamViecTrongTuan ASC, nd.HoTen ASC;

    SET @ThongBao = N'Tìm kiếm nhân viên thành công.';
    RETURN 0;
END
GO
